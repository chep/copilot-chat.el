...
In v2, only one buffer is used for chat, thanks to polymode. For the moment, org is the only frontend supported but markdown will follow. Anyway, you should use org since it is so powerful.

For the moment, shell-maker frontend has been completely removed. It uses specific functions that won't be ported in the initial v2 release.
...
** Frontend
Two frontends are available. You can select your favorite one by setting the ~copilot-chat-frontend~ variable to ~'org~ (default) or ~'markdown~.
