...
#+begin_src emacs-lisp
(add-hook 'git-commit-setup-hook 'copilot-chat-insert-commit-message-no-clobber)
#+end_src
...
- ~(copilot-chat-insert-commit-message-no-clobber)~ Like ~copilot-chat-insert-commit-message~, but skip generation if the buffer already has a non-comment message. Useful for ~git-commit-setup-hook~ to avoid overwriting existing messages during amend, rebase, or squash.
